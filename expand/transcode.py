...
def count_time(name = 'Unknown'):
    def wrapper(func):
        async def adeco(*args, **kwargs):
            t = time.time()
            result = await func(*args, **kwargs)
            logger.info(f'{name}用时{round(time.time()-t,2)}s')
            return result
        def deco(*args, **kwargs):
            ...
            logger.info(f'{name}用时{round(time.time()-t,2)}s')
            return result
        return adeco if asyncio.iscoroutinefunction(func) else deco
    return wrapper
...
async def silk(data, mtype = 'b' , options = ''):
    ...
    cmd = [f'ffmpeg -i "{file}" {options} -af aresample=resampler=soxr '\
           ...
           f'"lib/silk_v3_encoder" cache.wav cache.slk -quiet -tencent']
